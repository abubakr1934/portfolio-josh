...
const AddSkillModal = ({ isOpen, onClose, onSave }) => {
  ...
  const [skills, setSkills] = useState(
    Array.from({ length: 5 }, () => ({ name: "", percentage: "" }))
  );
  ...
  const handleSubmit = (e) => {
    ...
    if (domain && isValid) {
      ...
      alert(
        "Please fill in the domain and at least one skill with its proficiency."
      );
    }
  };
  ...
  return (
    <div
      className={`fixed inset-0 z-50 ${
        !isOpen && "hidden"
      } flex items-center justify-center bg-gray-600 bg-opacity-65`}
    >
      <div className="bg-white p-6 rounded-lg shadow-lg w-[50%]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-4">
            <button
              type="submit"
              className="bg-yellow-500 hover:bg-yellow-700 text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-28"
            >
              Add Skill
            </button>
            <button
              type="button"
              onClick={onClose}
              className="bg-white text-red-500 border border-red-500 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-28"
            >
              Cancel
            </button>
          </div>
